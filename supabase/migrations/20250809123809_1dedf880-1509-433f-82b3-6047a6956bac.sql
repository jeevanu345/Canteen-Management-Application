-- Create a public storage bucket for menu item images and secure RLS policies
...
BEGIN
  IF EXISTS (
    ...
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Public can view menu images'
  ) THEN
    DROP POLICY "Public can view menu images" ON storage.objects;
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Staff can upload menu images'
  ) THEN
    DROP POLICY "Staff can upload menu images" ON storage.objects;
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Staff can update own menu images'
  ) THEN
    DROP POLICY "Staff can update own menu images" ON storage.objects;
  END IF;
  IF EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'storage' AND tablename = 'objects' AND polname = 'Staff can delete own menu images'
  ) THEN
